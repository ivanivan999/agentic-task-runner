...
export default function App() {
  ...
  const liveTask: TaskRecord | undefined = pendingInput ? { id: "live", input: pendingInput, output: liveSteps.at(-1)?.description ?? "Selecting a tool…", toolsUsed: [], steps: liveSteps, timestamp: new Date().toISOString(), status: "pending", ownerId: clientId } : undefined;
  return <div className="app-shell"><ActivitySidebar tasks={tasks} isAdmin={role === "admin"} onDelete={remove}/><main className="chat"><header className="chat-header"><a className="wordmark" href="/">Agentic Task Runner</a><RoleSwitcher role={role} clientId={clientId} onChange={setRole} onNewUser={newUser}/></header><section className="thread" role="log" aria-live="polite">{tasks.length === 0 && !pendingInput && <div className="empty-state"><h1>What should I run?</h1><p>Try a calculation, some text processing, or a city’s weather.</p></div>}{tasks.map(task => <div className="turn" id={task.id} key={task.id}><UserBubble input={task.input}/><AgentBubble task={task}/></div>)}{pendingInput && <div className="turn live-turn"><UserBubble input={pendingInput} live/>{liveTask && <AgentBubble task={liveTask} live/>}</div>}<div ref={endOfThread}/></section>{error && <p className="error" role="alert">{error}</p>}<footer className="composer-area"><Composer onSubmit={submit} loading={loading}/><p>Each task runs independently. <span>Flight Recorder shows the routing trace.</span></p></footer></main></div>;
}
