...
export function ActivitySidebar({ tasks, isAdmin, onDelete }: { tasks: TaskRecord[]; isAdmin: boolean; onDelete: (task: TaskRecord) => void }) {
  ...
  const group = (label: string, records: TaskRecord[]) => records.length ? <section><h3>{label}</h3>{records.map(task => <div className="activity-item" key={task.id}><a href={`#${task.id}`}>{task.input}</a>{isAdmin && <button onClick={() => onDelete(task)} aria-label={`Delete ${task.input}`}>×</button>}</div>)}</section> : null;
